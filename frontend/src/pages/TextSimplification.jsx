...
const TextSimplification = () => {
    ...
    useEffect(() => {
        if (!mountedRef.current) {
            speak("Text reading mode opened");
    ...
    const handleCameraClick = async () => {
        if (!cameraActive) {
            await startCamera();
            speak("Camera active. Tap again to read text.");
        } else {
            captureAndAnalyze();
        }
    };
    ...
    const captureAndAnalyze = () => {
        ...
        canvas.toBlob(async (blob) => {
            ...
            formData.append('image', blob, 'text.jpg');
            console.log("Sending Text API Request...");

            try {
                const data = await api.analyzeText(formData);
    ...
    return (
        <div className="min-h-[calc(100vh-64px)] bg-gray-50 dark:bg-gray-900 flex flex-col p-4 pt-24">
            <div className="text-center pb-6 space-y-2">
                <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white">Read Text</h1>
                <p className="text-gray-500 dark:text-gray-400 text-sm font-medium">Point the camera at text to hear it read aloud</p>
                {cameraActive && <p className="text-green-500 animate-pulse font-bold">Live Camera Active</p>}
            </div>
            ...
            <div className="flex-grow flex flex-col items-center justify-start space-y-4 relative">
                ...
                {!loading && !result && (
                    <button
                        ...
                        ${cameraActive ? 'bg-white ring-8 ring-green-500 hover:scale-105' : 'bg-green-600 hover:bg-green-700'}`}
                    >
                        {cameraActive ? (
                ...
                {loading && (
                    <div className="text-center space-y-4">
                        <LoadingSpinner />
                        <p className="text-lg font-medium text-gray-600 dark:text-gray-300">Reading Text...</p>
                    </div>
                )}
                    ...
                    <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-3xl shadow-xl overflow-hidden p-6 space-y-6 animate-slide-up">
                        ...
                        <div className="flex gap-4">
                            <button
                                ...
                                className="flex-1 py-4 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded-2xl font-bold flex items-center justify-center gap-2 hover:opacity-80 transition-opacity"
                            >
                            ...
                            >
                                <RotateCcw className="w-6 h-6" /> Read New
                            </button>
                        </div>
                    </div>
                )}
